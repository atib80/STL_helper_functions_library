...
namespace cpp
{
	namespace experimental
	{
		...
		struct is_anyone_of<T, First, Rest...>
			: std::integral_constant<bool, std::is_same_v<T, First> || is_anyone_of<T, Rest...>::value>
		{
		};
		...
		template <typename CharacterPointerType,
		          std::enable_if_t<
			          is_anyone_of_v<CharacterPointerType, const char*, const wchar_t*, const char16_t*,
			                                    const char32_t*, char*, wchar_t*, char16_t*, char32_t*>, void*> 
			          =
			          nullptr>
		...
		template <typename StringType,
		          std::enable_if_t<is_anyone_of_v<StringType, std::string, std::wstring, std::u16string, std::u32string>, void*>  = nullptr>
		size_t str_length(StringType&& str,
		                  const size_t max_allowed_string_length = max_string_length)
		{
			return str.size() > max_allowed_string_length ? max_allowed_string_length : str.size();
		}

		template <typename CharPointerType,
		          std::enable_if_t<std::is_array_v<CharPointerType> ||
		                           is_anyone_of_v<CharPointerType, char*, wchar_t*, char16_t*, char32_t*>, void*>  = nullptr>

		bool trim(CharPointerType src)
		{
			using char_type = std::remove_extent_t<std::remove_pointer_t<CharPointerType>>;
		...
		template <typename CharPointerType,
		          std::enable_if_t<std::is_array_v<CharPointerType> ||
		                           is_anyone_of_v<CharPointerType, char*, wchar_t*, char16_t*, char32_t*>, void*>>
		...
		template <typename ConstCharPointerType,
		          std::enable_if_t<std::is_array_v<ConstCharPointerType> || is_anyone_of_v<ConstCharPointerType, const char*, const wchar_t*, const char16_t*, const char32_t*>, void*>>

		std::basic_string<std::remove_cv_t<std::remove_extent_t<std::remove_pointer_t<ConstCharPointerType>>>>
		trim(ConstCharPointerType src)
		{
			using char_type = std::remove_cv_t<std::remove_extent_t<std::remove_pointer_t<ConstCharPointerType>>>;
		...
		template <typename ConstCharPointerType,
		          std::enable_if_t<(std::is_const_v<ConstCharPointerType> && std::is_array_v<ConstCharPointerType>) || is_anyone_of_v<ConstCharPointerType, const char*, const wchar_t*, const char16_t*, const char32_t*>, void*>>
		...
		template <
			...
			std::enable_if_t<std::is_array_v<CharPointerType> || is_anyone_of_v<CharPointerType, const char*, const wchar_t*, const char16_t*, const char32_t*, char*, wchar_t*, char16_t*, char32_t*>,
			                 void*>>
		bool str_starts_with(
			CharPointerType src,
			const std::remove_cv_t<std::remove_extent_t<std::remove_pointer_t<CharPointerType>>> start_char,
			bool ignore_case = false,
			const std::locale& loc = std::locale{})
		{
			using char_type = std::remove_cv_t<std::remove_extent_t<std::remove_pointer_t<CharPointerType>>>;
		...
		bool str_starts_with(CharPointerType src,
		                     std::add_cv_t<std::add_pointer_t<std::remove_extent_t<std::remove_pointer_t<CharPointerType>>>> start_tag,
		                     bool ignore_case = false,
		                     const std::locale& loc = std::locale{})
		{
			return true;
		...
		template <typename StringType,
		          std::enable_if_t<is_anyone_of_v<StringType, std::string, std::wstring, std::u16string, std::u32string>,
		                           void*>>
		                     ...
		                     const std::locale& loc = std::locale{})
		{
			...
			{
				return static_cast<char_type>(std::tolower(ch, loc));
			});

			std::transform(cbegin(start_tag_str), cend(start_tag_str),
			               begin(start_tag_lc), [&loc](const auto ch)
			               {
				               return static_cast<char_type>(std::tolower(ch, loc));
			               });

			if (0u == src_lc.find(start_tag_lc))
				return true;
		...
		template <
			...
			std::enable_if_t<std::is_array_v<CharPointerType> || is_anyone_of_v<CharPointerType, const char*, const wchar_t*, const char16_t*, const char32_t*, char*, wchar_t*, char16_t*, char32_t*>,
			                 void*>>
		size_t str_index_of(CharPointerType src,
		                    std::remove_cv_t<std::remove_extent_t<std::remove_pointer_t<CharPointerType>>> needle_char,
		                    size_t start_pos = 0u,
		                    bool ignore_case = false,
		                    const std::locale& loc = std::locale{})
		{
			...
			const std::basic_string<char_type> src_str{src};
		  ...
		  std::basic_string<char_type> text_lc{src};

		  std::transform(std::cbegin(src_str), std::cend(src_str), std::begin(text_lc),
		            [&loc](const char ch) { return std::tolower(ch, loc); });

		  return text_lc.find(std::tolower(needle_char, loc), start_pos);			
		}

		size_t str_index_of(const char* text,
		                    const char* needle,
		                    size_t start_pos = 0u,
		                    bool ignore_case = false,
		                    const std::locale& loc = std::locale{});
		size_t str_index_of(const wchar_t* text,
		                    const wchar_t* needle,
		                    size_t start_pos = 0u,
		                    bool ignore_case = false,
		                    const std::locale& loc = std::locale{});
		size_t str_index_of(const char16_t* text,
		                    const char16_t* needle,
		                    size_t start_pos = 0u,
		                    bool ignore_case = false,
		                    const std::locale& loc = std::locale{});
		size_t str_index_of(const char32_t* text,
			...
			const std::locale& loc = std::locale{})
		{
			...
			transform(
				cbegin(needle_str), cend(needle_str), begin(needle_lc),
				[&loc](const auto ch)
				{
					return static_cast<typename StringType::value_type>(tolower(ch, loc));
				});
		...
		StringType rtrim(const StringType& str)
		{
			...
			for (auto is_ws_char{true}; (is_ws_char && (end != StringType::npos));
			     --end)
			{
				switch (static_cast<char_type>(str[end]))
				{
