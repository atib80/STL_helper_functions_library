...
template <typename CharacterPointerType,
          std::enable_if_t<is_anyone_of_v<CharacterPointerType,
                                          const char*,
                                          const wchar_t*,
                                          const char16_t*,
                                          const char32_t*,
...
template <typename StringType,
          std::enable_if_t<is_anyone_of_v<StringType,
                                          std::string,
                                          std::wstring,
                                          std::u16string,
                                          std::u32string>,
...
size_t str_length(StringType&& str,
                  const size_t max_allowed_string_length = max_string_length) {
  return str.size() > max_allowed_string_length ? max_allowed_string_length
                                                : str.size();
}
...
bool trim(CharPointerType src) {
  using char_type =
      std::remove_extent_t<std::remove_pointer_t<CharPointerType>>;
...
std::basic_string<std::remove_cv_t<
    std::remove_extent_t<std::remove_pointer_t<ConstCharPointerType>>>>
...
bool str_starts_with(CharPointerType src,
                     const std::remove_cv_t<std::remove_extent_t<
                         std::remove_pointer_t<CharPointerType>>> start_char,
...
template <typename CharPointerType,
          std::enable_if_t<std::is_array_v<CharPointerType> ||
                           ...
                           void*>>
bool str_starts_with(CharPointerType src,
                     std::add_cv_t<std::add_pointer_t<std::remove_extent_t<
                         std::remove_pointer_t<CharPointerType>>>> start_tag,
                     bool ignore_case = false,
                     const std::locale& loc = std::locale{}) {
  return true;
}
...
size_t str_index_of(CharPointerType src,
                    std::remove_cv_t<std::remove_extent_t<
                        std::remove_pointer_t<CharPointerType>>> needle_char,
                    size_t start_pos = 0u,
                    bool ignore_case = false,
                    const std::locale& loc = std::locale{}) {
  using char_type = std::remove_cv_t<
