...
// various useful generic algorithms (function templates)
        ...
        struct get_char_type {
            using char_type = std::remove_cv_t<
                    std::remove_pointer_t<std::decay_t<std::remove_cv_t<T>>>>;
            // static_assert(is_anyone_of_v<type, char, wchar_t, char16_t, char32_t>);
            using type = std::conditional<is_anyone_of_v<char_type, char, wchar_t, char16_t, char32_t>, char_type, std::false_type>;
        };
        ...
        bool str_contains(T src,
            ...
            if constexpr (is_valid_char_type_v<U>) {
                ...
                return std::basic_string<char_type>::npos != src_str.find(f.tolower(needle), start_pos);
                ...
                if (!ignore_case)
                    return std::basic_string_view<char_type>::npos != src_str_view.find(needle_str_view, start_pos);
                ...
                return std::basic_string<char_type>::npos != src_str.find(needle_str, start_pos);
            }
        ...
        bool str_contains(const StringType &src,
            ...
            transform(cbegin(src), cend(src), begin(src_lc), [&f](const auto ch) {
                return f.tolower(ch);
            });
        ...
        bool str_contains(const StringType &src,
            ...
            transform(
                    cbegin(needle), cend(needle), begin(needle_lc), [&f](const auto ch) {
                        return f.tolower(ch);
                    });
            ...
            if constexpr (is_valid_char_type_v<get_char_type_t<U>>) {
                ...
                if (!ignore_case) {
                    for (size_t i{expected_start_pos_of_end_tag}, j{}; i < src_len; i++, j++) {
                        if (src[i] != needle[j]) return false;
                    }
                ...
                for (size_t i{expected_start_pos_of_end_tag}, j{}; i < src_len; i++, j++) {
                    if (f.tolower(src[i]) != f.tolower(needle[j])) return false;
                }
        ...
        bool str_ends_with(const StringType &text,
            ...
            const auto &f = std::use_facet<std::ctype<typename StringType::value_type>>(loc);
