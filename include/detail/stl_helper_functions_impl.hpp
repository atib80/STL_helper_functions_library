...
#error \
  "You need a modern compiler that supports at least the C++20 language standard or a newer one in order to be able to use this header-only library!"
#endif
...
concept are_same = std::conjunction_v<std::is_same<T, T0_TN>...>;
...
template<typename T, typename T0, typename... T1_TN>
static constexpr bool is_anyone_of_v = is_anyone_of_impl<T, T0, T1_TN...>::value;

template<typename T>
concept valid_char_type = is_anyone_of_v<std::remove_cv_t<T>,
  char,
  wchar_t,
  char16_t,
  char32_t,
