...
public:
  ...
  {
    constexpr size_t buffer_len{ 1024U };
...
#define PRINT_VAR_NAME(output_stream, arg) (output_stream << #arg << ' ')
#define PRINT_VAR_NAMEW(wide_output_stream, arg) (wide_output_stream << #arg << L' ')
...
template<typename T>
concept has_key_type = requires(const T &) {
  typename T::key_type;
};

template<typename T>
concept has_value_type = requires(const T &) {
...
struct has_value_type_meta : std::false_type
{
};

template<typename T>
struct has_value_type_meta<T, std::void_t<has_value_type_t<T>>> : std::true_type
{
};

template<typename T>
constexpr bool has_value_type_v = has_value_type_meta<T>::value;

template<typename T>
concept has_insert_member_func = requires(T &t) {
  typename T::value_type;
  t.insert(std::declval<typename T::value_type>());
};

template<typename T>
concept has_mapped_type = requires(const T &) {
  typename T::mapped_type;
};

template<typename T>
concept has_sort_member_function = requires(T &t) {
  t.sort();
};

template<typename T, typename U>
concept has_find_member_function = requires(const T &t, U u) {
  t.find(u);
};

template<typename StreamType, typename T>
concept has_output_stream_operator = requires(StreamType &stream, const T &value) {
  stream << value;
};

template<typename T>
concept has_operator_dereference = requires(T t) {
  *t;
  t.operator*();
};

template<typename T, typename U = T>
concept has_operator_not_equals = requires(const T &t, const U &u) {
  { t != u } -> std::convertible_to<bool>;
};

template<typename T, typename U = T>
concept has_operator_equals = requires(const T &t, const U &u) {
  { t == u } -> std::convertible_to<bool>;
};

template<typename T, typename U = T>
concept has_operator_less_than = requires(const T &t, const U &u) {
  { t < u } -> std::convertible_to<bool>;
};

template<typename T>
concept has_begin_member_function = requires(const T &obj) {
  obj.begin();
};

template<typename T>
concept has_end_member_function = requires(const T &obj) {
  obj.end();
};

template<typename T>
concept has_cbegin_member_function = requires(const T &obj) {
  { obj.cbegin() };
};

template<typename T>
concept has_cend_member_function = requires(const T &obj) {
  { obj.cend() };
};
...
void show_var_info(std::ostream &os, const T &arg, const Args &...args)
{
  os << "Name: ";
  PRINT_VAR_NAME(os, arg);
  os << "\nType: " << get_type_name(arg) << "\nValue: " << arg << '\n';
  if constexpr (sizeof...(Args) > 0u) {
    show_var_info(os, args...);
  }
}
...
public:
  template<typename RunnableType>
    requires std::convertible_to<RunnableType, Runnable>
  explicit run_at_scope_exit(RunnableType callable_object)
    : callable{ std::move(callable_object) }
  {
  }
...
template<typename Runnable>
run_at_scope_exit(Runnable) -> run_at_scope_exit<Runnable>;
...
public:
  explicit run_task_at_scope_exit(Runnable... callable_objects)
    : callable_tasks{ std::move(callable_objects)... }
  {
  }
...
template<typename... Runnable>
run_task_at_scope_exit(Runnable...) -> run_task_at_scope_exit<Runnable...>;

template<typename... Runnable>
auto create_tasks_to_run_at_scope_exit(Runnable... tasks)
{
  return run_task_at_scope_exit<Runnable...>{ std::move(tasks)... };
}
...
template<detail::valid_char_type CharType>
...
template<typename T>
concept non_const_char_pointer_type = detail::is_anyone_of_v<T,
  char *,
  char8_t *,
  char8_t *const,
  wchar_t *,
...
template<typename T>
concept const_char_pointer_type = detail::is_anyone_of_v<T,
  const char *,
  const char8_t *,
  const char8_t *const,
  const wchar_t *,
...
template<typename T>
concept non_const_char_array_type = (1U == std::rank_v<T> && detail::valid_char_type<std::remove_extent_t<T>> && !std::is_const_v<std::remove_extent_t<T>>);

template<typename T>
concept const_char_array_type = (1U == std::rank_v<T> && detail::valid_char_type<std::remove_extent_t<T>> && std::is_const_v<std::remove_extent_t<T>>);

template<typename T>
concept char_pointer_type = non_const_char_pointer_type<T> || const_char_pointer_type<T>;

template<typename T>
concept char_array_type = non_const_char_array_type<T> || const_char_array_type<T>;
...
template<typename T>
concept valid_string_type = detail::is_anyone_of_v<remove_all_decorations_t<T>,
  std::string,
  std::wstring,
  std::u16string,
  std::u32string,
...
template<typename T>
concept valid_string_view_type = detail::is_anyone_of_v<remove_all_decorations_t<T>,
  std::string_view,
  std::wstring_view,
  std::u16string_view,
  std::u32string_view,
  std::u8string_view>;
template<typename T>
concept is_char_type = detail::is_anyone_of_v<T, char, char8_t, wchar_t, char16_t, char32_t, const char, const char8_t, const wchar_t, const char16_t, const char32_t>;

template<typename T>
concept char_pointer_or_char_array_type = char_pointer_type<T> || char_array_type<T>;

template<typename T, typename = void>
struct get_char_type
{
  using type = remove_all_decorations_t<T>;
  static_assert(detail::valid_char_type<type>,
...
template<detail::valid_char_type T,
  size_t ARRAY_SIZE>
...
constexpr size_t len(T src)
  requires(std::same_as<T, std::nullptr_t> || char_pointer_type<T> || valid_string_view_type<T> || detail::valid_char_type<T>)
{
  if constexpr (std::same_as<T, std::nullptr_t>) {
    unused_args(src);
    return 0U;
  } else if constexpr (detail::valid_char_type<T>) {
    unused_args(src);
    return 1U;
  } else if constexpr (valid_string_view_type<T>)
    return src.length();
...
size_t say(std::ostream &os, const char *format_string, Args &&...args)
{
  ...
  std::scoped_lock guard{ say_cstr_mu };
...
size_t say(std::wostream &os, const wchar_t *format_string, Args &&...args)
{
  ...
  std::scoped_lock guard{ say_wstr_mu };
...
template<typename T>
  requires(const_char_array_type<T> || non_const_char_pointer_type<T>)
bool trim_in_place(
  T src,
  const add_const_pointer_to_char_type_t<get_char_type_t<T>> chars_to_trim =
    stl::helper::default_whitespace_chars_v<get_char_type_t<T>>)
{
...
template<typename T>
  requires(non_const_char_array_type<T> || non_const_char_pointer_type<T>)
bool ltrim_in_place(
  T src,
...
template<typename T>
  requires(char_array_type<T> || char_pointer_type<T>)
    ...
    default_whitespace_chars_v<get_char_type_t<T>>)
{
  ...
  if constexpr (char_pointer_type<T>) {
    if (nullptr == src)
      return {};
  }
  ...
  const auto first{
    std::find_if(std::cbegin(source_str), std::cend(source_str), [&trimmed_chars](const auto ch) {
      return trimmed_chars.find(ch) == std::cend(trimmed_chars);
    })
  };
  ...
  const auto last{
    std::find_if(std::crbegin(source_str), std::crend(source_str), [&trimmed_chars](const auto ch) {
      return trimmed_chars.find(ch) == std::cend(trimmed_chars);
    }).base()
  };
...
template<typename T>
  requires(valid_string_type<T> || valid_string_view_type<T>)
std::basic_string<get_char_type_t<T>> trim(
  const T &src,
  add_const_pointer_to_char_type_t<get_char_type_t<T>> chars_to_trim =
    default_whitespace_chars_v<get_char_type_t<T>>)
    ...
    default_whitespace_chars_v<get_char_type_t<T>>)
{
  ...
  const auto last{
    std::find_if(std::rbegin(source_str), std::rend(source_str), [&trimmed_chars](const auto ch) {
...
template<std::ranges::forward_range SrcIterType,
  typename DstIterType>
  requires std::same_as<typename std::iterator_traits<SrcIterType>::value_type, typename std::iterator_traits<DstIterType>::value_type>
  ...
  const DstIterType dst_last)
{
  ...
  if constexpr (std::ranges::random_access_range<DstIterType> && hashable<typename std::iterator_traits<DstIterType>::value_type>) {
    return src_first == std::search(src_first, src_last, std::boyer_moore_searcher(dst_first, dst_last));
  }

  return src_first == std::search(src_first, src_last, dst_first, dst_last);
}

template<std::ranges::input_range SrcIterType,
  std::ranges::forward_range DstIterType>
  requires std::same_as<typename std::iterator_traits<
                          SrcIterType>::value_type,
...
template<
  typename T,
  typename U>
  requires((char_array_type<T> || char_pointer_type<T> || valid_string_type<T> || valid_string_view_type<T>) && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U> || detail::valid_char_type<U>) && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
  ...
  const std::locale &loc = std::locale{})
{
  ...
  if constexpr (detail::valid_char_type<U>) {
    if (!ignore_case)
      return needle == src[0];
...
template<
  typename T,
  typename U>
  requires(char_array_type<T> || char_pointer_type<T> || valid_string_type<T> || valid_string_view_type<T>) && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U> || detail::valid_char_type<U>) && std::is_same_v<get_char_type_t<T>, get_char_type_t<U>>
  ...
  const std::locale &loc = std::locale{})
{
  ...
  if constexpr (char_pointer_type<T> || char_array_type<T>)
    src_sv = { src, src_len };
  else
    src_sv = src;
  ...
  if constexpr (detail::valid_char_type<U>) {
    ...
    if constexpr (char_pointer_type<U> || char_array_type<U>)
  ...
  const std::locale &loc = std::locale{})
{
  ...
  } else if constexpr (char_pointer_type<U> || char_array_type<U>)
    needle_sv = { needle, needle_len };
  else
    needle_sv = needle;
  ...
  const DstIterType dst_last)
{
  ...
  if constexpr (detail::is_anyone_of_v<typename std::iterator_traits<
    ...
    const bool is_haystack_sorted = false)
{
  ...
  if constexpr (stl::helper::has_find_member_function<ContainerType, T>) {
    const auto found{
  ...
  ContainerType &haystack)
{
  ...
  bool is_haystack_sorted{
    has_find_member_function<ContainerType, T>
  };

  if constexpr (!has_find_member_function<ContainerType, T> && ((has_sort_member_function<ContainerType> && has_operator_less_than<U>) || has_operator_less_than<U>)) {
    if constexpr (has_sort_member_function<ContainerType>)
      haystack.sort();
    else
...
template<typename T, typename... Args>
concept is_container_adapter_type = std::same_as<T, std::stack<Args...>> || std::same_as<T, std::queue<Args...>>
                                    || std::same_as<T, std::priority_queue<Args...>>;

template<typename ContainerType,
  typename KeyType>
  requires has_find_member_function<ContainerType, KeyType>
bool has_key(const ContainerType &container, const KeyType &key)
{
  return std::cend(container) != container.find(key);
}

template<
  typename ContainerType,
  typename ValueType>
  requires(!is_container_adapter_type<ContainerType> && has_begin_member_function<ContainerType> && has_end_member_function<ContainerType> && (has_key_type<ContainerType> || has_value_type<ContainerType> || has_mapped_type<ContainerType>))
bool has_value(const ContainerType &container, const ValueType &value)
{
  if constexpr (has_mapped_type<ContainerType>) {
    ...
    return false;
  } else if constexpr (has_key_type<ContainerType>) {
    for (const auto &element : container) {
      if (element == value) {
        return true;
      }
    ...
    return false;
  } else if constexpr (has_value_type<ContainerType>) {
    if constexpr (has_operator_less_than<
...
bool has_value(const std::array<T, N> &container, const U &value)
{
  if constexpr (has_operator_less_than<T>) {
  ...
  return std::cend(container) != std::find(std::cbegin(container), std::cend(container), value);
}
...
template<typename T,
  ...
  typename U>
  requires std::convertible_to<U, T>
bool has_value(const T (&arr)[N], const U &value)
{
  if constexpr (has_operator_less_than<T>) {
    if (std::is_sorted(arr, arr + N)) {
      return std::binary_search(arr, arr + N, value);
    }
  }

  return arr + N != std::find(arr, arr + N, value);
}

template<typename ContainerType, typename KeyType, typename ValueType>
  requires(
    has_key_type<ContainerType> && has_value_type<ContainerType> && has_mapped_type<ContainerType> && std::convertible_to<KeyType, typename ContainerType::key_type> && std::convertible_to<ValueType, typename ContainerType::mapped_type>)
bool has_kv_pair(const ContainerType &container,
  const std::pair<KeyType, ValueType> &key_value_pair)
{
  ...
  for (auto current{ first_item_iter_pos.first };
    first_item_iter_pos.second != current;
    ++current) {
...
template<typename ForwardIterType,
  typename ItemType>
  requires std::is_convertible_v<std::remove_reference_t<ItemType>, typename std::iterator_traits<ForwardIterType>::value_type>
bool has_item(const ForwardIterType first,
  const ForwardIterType last,
  ItemType &&item)
{
  if constexpr (has_operator_less_than<typename std::iterator_traits<
...
template<typename T,
  typename U>
  requires((char_array_type<T> || char_pointer_type<T> || valid_string_type<T> || valid_string_view_type<T>) && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>) && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<
  ...
  requires std::same_as<get_char_type_t<typename std::iterator_traits<
                          FwIterType1>::value_type>,
...
template<
  ...
  size_t ARRAY_SIZE,
  typename U>
  requires(detail::valid_char_type<T> && !std::is_const_v<T> && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>)
           && std::same_as<std::remove_cv_t<T>, get_char_type_t<U>>)
...
template<
  typename T,
  typename U>
  requires(non_const_char_pointer_type<T> && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>) && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
size_t str_copy(
  T dst,
...
template<typename T,
  typename U>
  requires(valid_string_type<T> && !std::is_const_v<T> && (valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U>) && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<typename T,
  typename U,
  size_t ARRAY_SIZE>
  requires(valid_string_type<T> && !std::is_const_v<T> && detail::valid_char_type<U> && std::same_as<get_char_type_t<T>, std::remove_cv_t<U>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  if constexpr (char_pointer_type<U>)
    src_sv = { src, src_len };
  else
    src_sv = src;
...
template<typename T,
  typename U>
  requires((valid_string_type<T> || valid_string_view_type<T> || char_pointer_type<T> || char_array_type<T>) && (valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>) && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  if constexpr (valid_string_type<T> || valid_string_view_type<T>)
    dst_sv = dst;
  else
    dst_sv = { dst, dst_len };
  ...
  if constexpr (valid_string_type<U> || valid_string_view_type<U>)
    src_sv = src;
  else
    src_sv = { src, src_len };
...
template<typename T,
  typename U>
  requires(valid_string_type<T> && !std::is_const_v<T>
           && (valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
size_t str_append(T &dst,
  const U &src,
  size_t *required_dst_capacity = nullptr)
{
...
template<typename T,
  typename U>
  requires(valid_string_type<T> && !std::is_const_v<T> && (valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
size_t str_append_n(T &dst,
  const U &src,
  size_t number_of_characters_to_append =
...
template<
  typename T,
  typename U>
  requires(non_const_char_pointer_type<T> && (char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<typename T,
  typename U>
  requires((valid_string_type<T> || valid_string_view_type<T> || char_pointer_type<T> || char_array_type<T>)
           && (valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  if (0U == dst_len)
    return std::basic_string<char_type>{
      std::cbegin(src_sv) + std::cbegin(src_sv) + number_of_characters_to_prepend
    };
...
template<
  ...
  typename U,
  typename V>
  requires(detail::valid_char_type<T> && !std::is_const_v<T>
           && (detail::valid_char_type<U> || char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>) && (detail::valid_char_type<V> || char_array_type<V> || char_pointer_type<V> || valid_string_type<V> || valid_string_view_type<V>)
           && detail::are_same<std::remove_cv_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  if constexpr (char_pointer_type<V>) {
    if (nullptr == replace)
      return 0U;
  }
  ...
  if constexpr (detail::valid_char_type<V>) {
    replace_buffer[0] = replace;
    replace_sv = { replace_buffer, 1U };
  } else if constexpr (char_pointer_type<V> || char_array_type<V>)
...
template<
  ...
  typename U,
  typename V>
  requires(non_const_char_pointer_type<T>
           && (detail::valid_char_type<U> || char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>) && (detail::valid_char_type<V> || char_array_type<V> || char_pointer_type<V> || valid_string_type<V> || valid_string_view_type<V>)
           && detail::are_same<get_char_type_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
size_t str_replace_first(
  T dst,
  const size_t dst_capacity_in_number_of_characters,
  const U &needle,
...
template<
  ...
  typename V>
  requires((
             valid_string_type<T> || valid_string_view_type<T> || char_pointer_type<T> || char_array_type<T>)
           && (detail::valid_char_type<U> || valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>) && (detail::valid_char_type<V> || valid_string_type<V> || valid_string_view_type<V> || char_pointer_type<V> || char_array_type<V>) && detail::are_same<get_char_type_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  if constexpr (char_pointer_type<V> || const_char_array_type<V>)
    replace_sv = { replace, replace_len };

  else if constexpr (detail::valid_char_type<V>) {
    replace_buffer[0] = replace;
    replace_sv = { replace_buffer, 1U };
  } else
    replace_sv = replace;
...
template<
  ...
  typename V>
  requires(valid_string_type<T> && !std::is_const_v<T>
           && (detail::valid_char_type<U> || valid_string_type<U> || valid_string_view_type<U> || char_pointer_type<U> || char_array_type<U>)
           && (detail::valid_char_type<V> || valid_string_type<V> || valid_string_view_type<V> || char_pointer_type<V> || char_array_type<V>)
           && detail::are_same<get_char_type_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
size_t str_replace_first(
  T &dst,
  const U &needle,
...
template<
  ...
  typename U,
  typename V>
  requires(detail::valid_char_type<T> && !std::is_const_v<T> && (detail::valid_char_type<U> || char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>)
           && (detail::valid_char_type<V> || char_array_type<V> || char_pointer_type<V> || valid_string_type<V> || valid_string_view_type<V>)
           && detail::are_same<std::remove_cv_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
  ...
  size_t *required_dst_capacity = nullptr)
{
  ...
  do {
    const size_t next_pos{
      str_index_of(dst_sv, needle_sv, nth_needle_pos, ignore_case_when_searching_for_needle)
    };
...
template<
  ...
  typename V>
  requires(non_const_char_pointer_type<T>
           && (detail::valid_char_type<U> || char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>)
           && (detail::valid_char_type<V> || char_array_type<V> || char_pointer_type<V> || valid_string_type<V> || valid_string_view_type<V>)
           && detail::are_same<get_char_type_t<T>, get_char_type_t<U>, get_char_type_t<V>>)
...
template<typename T,
  ...
  requires(detail::valid_char_type<T> && !std::is_const_v<T>
           && (detail::valid_char_type<U> || char_array_type<U> || char_pointer_type<U> || valid_string_type<U> || valid_string_view_type<U>)
           && detail::are_same<std::remove_cv_t<T>, get_char_type_t<U>>)
size_t str_replace_last_n(
  T (&dst)[ARRAY_SIZE],
  const U &needle,
  const size_t last_n_needle_count,
...
template<typename T,
  typename U>
  requires((char_pointer_type<T> || char_array_type<T>)
           && (char_pointer_type<U> || char_array_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<
  ...
  typename U>
  requires((valid_string_type<T> || valid_string_view_type<T>)
           && (valid_string_type<U> || valid_string_view_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<
  ...
  typename U>
  requires((char_array_type<T> || char_pointer_type<T>)
           && (char_array_type<U> || char_pointer_type<U>)
           && std::same_as<get_char_type_t<T>, get_char_type_t<U>>)
...
template<typename T>
  requires(char_array_type<T> || char_pointer_type<T> || valid_string_type<T> || valid_string_view_type<T>)
...
template<typename T>
  requires(non_const_char_array_type<T> || non_const_char_pointer_type<T> || valid_string_type<T>)
...
template<typename T>
  requires(char_array_type<T> || char_pointer_type<T> || valid_string_type<T>)
...
{
  using integral_type = std::remove_cvref_t<T>;
...
std::u16string to_u16string(
  ...
  int number_of_decimal_digits =
    std::numeric_limits<std::remove_cvref_t<T>>::digits10)
{
  using floating_type = std::remove_cvref_t<T>;
    ...
    std::numeric_limits<std::remove_cvref_t<T>>::digits10)
{
  ...
  constexpr size_t buffer_size{ 32U };
  ...
  size_t *pos = nullptr)
{
  ...
  if constexpr (char_array_type<T> || char_pointer_type<T>)
    src_sv = { src, src_len };
      ...
      if (src_sv[i] == static_cast<char_type>('.')) {
        ...
        if (fractional_part_found) {
          if ((src_sv[i] >= static_cast<char_type>('0') && src_sv[i] <= static_cast<char_type>('9'))
              || (src_sv[i] >= static_cast<char_type>('A') && src_sv[i] <= static_cast<char_type>('F')) || (src_sv[i] >= static_cast<char_type>('a') && src_sv[i] <= static_cast<char_type>('f'))) {
            number_value *= 16;
...
template<typename T>
  requires detail::valid_char_type<T>
...
template<
  ...
  requires(
    (valid_string_type<T> || valid_string_view_type<T> || char_array_type<T> || char_pointer_type<T>)
    && (valid_string_type<U> || valid_string_view_type<U> || char_array_type<U> || char_pointer_type<U> || detail::valid_char_type<U>)
    && (valid_string_type<V> || valid_string_view_type<V> || char_array_type<V> || char_pointer_type<V> || detail::valid_char_type<V> || std::same_as<V, std::nullptr_t>)
    && ((std::same_as<get_char_type_t<T>, get_char_type_t<U>>
  ...
  size_t const max_count = std::basic_string<get_char_type_t<T>>::npos)
{
  ...
  } else if constexpr (detail::valid_char_type<U>) {
    needle_buffer[0] = needle;
  ...
  if constexpr (!std::is_same_v<V, std::nullptr_t>) {
    if (needle_parts_separator_token_len > 0U) {
      if constexpr (char_pointer_type<V> || char_array_type<V>) {
        ...
        needle_parts_separator_token_sv = {
          needle_parts_separator_token_buffer,
          1U
        };
      } else {
...
template<
  ...
  requires detail::valid_char_type<typename std::iterator_traits<IteratorType>::value_type>
           && std::same_as<typename std::iterator_traits<IteratorType>::value_type, get_char_type_t<NeedleType>>
           && (std::same_as<NeedleSeparatorType, std::nullptr_t>
               || detail::are_same<typename std::iterator_traits<IteratorType>::value_type, get_char_type_t<NeedleType>, get_char_type_t<NeedleSeparatorType>>)
std::vector<
  std::basic_string<typename std::iterator_traits<IteratorType>::value_type>> str_split_range(IteratorType first,
  IteratorType last,
  ...
  const size_t max_count = std::string::npos)
{
  ...
  if constexpr (char_pointer_type<NeedleType>) {
    if (nullptr == needle)
      return {};
  }
  ...
  if constexpr (char_pointer_type<NeedleType> || char_array_type<NeedleType>) {
    needle_sv = { needle, needle_len };
    unused_args(needle_buffer);
  } else if constexpr (detail::valid_char_type<NeedleType>) {
    needle_buffer[0] = needle;
    needle_buffer[1] = static_cast<char_type>(0);
    needle_sv = { needle_buffer, 1U };
  } else {
  ...
  if constexpr (!std::is_same_v<NeedleSeparatorType, std::nullptr_t>) {
    if (needle_parts_separator_token_len > 0U) {
      if constexpr (char_pointer_type<NeedleSeparatorType> || char_array_type<NeedleSeparatorType>) {
        needle_parts_separator_token_sv = {
          needle_parts_separator_token,
          needle_parts_separator_token_len
        };
        unused_args(needle_parts_separator_token_buffer);
      } else if constexpr (detail::valid_char_type<NeedleSeparatorType>) {
        needle_parts_separator_token_buffer[0] = needle_parts_separator_token;
        needle_parts_separator_token_buffer[1] = static_cast<char_type>(0);
        needle_parts_separator_token_sv = {
          needle_parts_separator_token_buffer,
...
template<typename T,
  typename U>
  requires(detail::valid_char_type<T> || char_array_type<T> || char_pointer_type<T> || valid_string_type<T> || valid_string_view_type<T>)
...
template<typename ContainerType,
  typename NeedleType>
  requires(has_value_type<ContainerType> && (detail::valid_char_type<NeedleType> || char_pointer_type<NeedleType> || char_array_type<NeedleType> || valid_string_type<NeedleType> || valid_string_view_type<NeedleType>)
           && has_begin_member_function<ContainerType> && has_end_member_function<ContainerType> && has_output_stream_operator<std::basic_ostream<get_char_type_t<NeedleType>>, typename ContainerType::value_type>)
...
template<typename FwIterType, typename NeedleType>
  requires(detail::valid_char_type<NeedleType> || char_pointer_type<NeedleType> || char_array_type<NeedleType> || valid_string_type<NeedleType> || valid_string_view_type<NeedleType>) && has_output_stream_operator<std::basic_ostream<get_char_type_t<NeedleType>>, typename std::iterator_traits<FwIterType>::value_type>
