...
size_t str_prepend_n(char32_t* dst,
  ...
  if (prepend_options ==
    ...
    if (dst_capacity_in_number_of_characters <
        (number_of_characters_to_prepend + dst_len + 1)) {
      ret_val = 0u;
    } else {
      copy_backward(dst, dst + dst_len,
...
size_t str_replace_first(char* dst,
  ...
  if (needle_len < replace_len) {
    ...
    copy_backward(dst + start_pos + needle_len, dst + dst_len,
                  dst + dst_len + noctm);  // copy characters from range [dst +
                                           // start_pos + needle_len, dst +
                                           // dst_len) to [dst + start_pos +
                                           // needle_len + noctm, dst + dst_len
                                           // + noctm)
    dst[dst_len + noctm] = '\0';
